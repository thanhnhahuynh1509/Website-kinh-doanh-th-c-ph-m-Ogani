                    ...
                    <div class="col-sm-12">
                        <div class="card">
                            <div class="card-header">
                                <button onclick="window.location.href = '/admin/category/not-active'" class="btn btn-warning waves-effect waves-light"><i class="fa-solid fa-ban"></i> Không hoạt động</button>
                                <button onclick="window.location.href = '/admin/category/trash'" class="btn btn-danger waves-effect waves-light"><i class="fa-solid fa-trash"></i> Thùng rác</button>
                                <button onclick="window.location.href = '/admin/category/create'" class="btn btn-success waves-effect waves-light"><i class="fa-solid fa-plus"></i> Thêm</button>
                            </div>
                        </div>
                    </div>
                </div>
                                            ...
                                            <tbody>
                                                <c:forEach items="${requestScope.categories}" var="item">
                                                    <tr class="text-center" >
                                                        ...
                                                        <td class="align-middle">
                                                            <button class="btn btn-sm btn-round btn-success waves-effect waves-light js-not-active" data-id="${item.id}">
                                                                <i class="fa-solid fa-toggle-on"></i>
                                                            </button>
                                                            <button class="btn btn-sm btn-round btn-warning waves-effect waves-light" onclick="window.location.href = '/admin/category/update?id=${item.id}'">
    ...
    $(document).ready(function() {
       $(".js-not-active").on('click', function(e) {
           ...
           if(confirm("Bạn có chắc chắn muốn chuyển trạng thái của dữ liệu sang không hoạt động?")) {
               $.ajax({
                   url: '/admin/category/not-active?id=' + btn.attr("data-id"),
                   type: 'POST'
               }).done(function (data) {
        ...
        $(".js-trash").on('click', function(e) {
            ...
            if(confirm("Bạn có chắc chắn muốn chuyển dữ liệu vào thùng rác?")) {
                $.ajax({
                    url: '/admin/category/trash?id=' + btn.attr("data-id"),
                    type: 'POST'
                }).done(function (data) {
                    btn.closest("tr").remove();
                });
